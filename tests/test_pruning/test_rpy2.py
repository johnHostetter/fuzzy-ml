...
class TestRuleReduction(unittest.TestCase):
    """
    Test that rule reduction using rough set techniques accessible via rpy2 behave as intended.
    """
    ...
    def test_find_unique_premise_variables(self) -> None:
        """
        Test the discovery of unique variables in the rules' premises.
    ...
    def test_reduce_fuzzy_logic_rules_with_rough_sets(self) -> None:
        ...
        # the rules should match, but the result does not preserve ordering
        self.assertEqual(
            {
                frozenset(expected_rule.premise.indices[0])
                for expected_rule in expected_rules
            },
            {frozenset(actual_rule.premise.indices[0]) for actual_rule in actual_rules},
        )
        self.assertNotEqual(
            {
                tuple(expected_rule.premise.indices[0])
                for expected_rule in expected_rules
            },
            {tuple(actual_rule.premise.indices[0]) for actual_rule in actual_rules},
        )
        self.assertTrue(
